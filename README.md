...
- **Vector recall on synopses.** Each item's synopsis is embedded into a pgvector column; cosine similarity surfaces taste-matched library examples. From it the recommender also gets a live `taste_signal` — historical add-rates over the candidate's same-era cohort, cross-tabbed by nearest-synopsis-neighbour accept-rate × critic-rating presence (plus per-network/genre rates and a cast cross-reference against the added library), so a low raw count reads against the right baseline instead of as a veto.
