...
Use brave_search to find the pages you need, then web_fetch to read their content (Rotten Tomatoes and Metacritic are usually static; Reddit and IMDb are JavaScript-rendered — fetch them with render=true from the start). Pull quantitative scores from primary sources:
